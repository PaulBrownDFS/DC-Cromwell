...
import useGlobalStyles, { globalStyles as rawGlobalStyles } from './globalStyles';
import { fontFamily, colour } from '@dfs/component-library/styles/theme/config';
  ...
  return (
    <Container maxWidth="md" className={classes.root}>
      <div style={{ display: 'inline-block', fontFamily: fontFamily }}>
        <div style={{ display: 'inline-block', marginRight: '20px', fontWeight: 500 }}>
          <div style={visualSize}>Aa</div>
          <div>Poppins Semi-bold</div>
        </div>
        <div style={{ display: 'inline-block', marginRight: '20px', fontWeight: 400 }}>
          <div style={visualSize}>Aa</div>
      ...
      <hr />
      <div className={globalStyles.carouselPriceStrikethrough}>Carousel Product Price</div>
      <pre display="flex" flexWrap="wrap">
        {JSON.stringify(rawGlobalStyles.carouselPriceStrikethrough)}
      </pre>

      <Box className={`${globalStyles.inverse} ${classes.contrastContainer}`} p={6}>
        <div style={{ display: 'inline-block', fontFamily: fontFamily }}>
          <div style={{ display: 'inline-block', marginRight: '20px', fontWeight: 500 }}>
            <div style={visualSize}>Aa</div>
            <div>Poppins Semi-bold</div>
        ...
        <pre>{JSON.stringify({ ...overrideValues?.h1, color: colour.canvas })}</pre>
        <hr />
        <h2 style={{ color: colour.canvas }}>Heading 2</h2>
        <pre>{JSON.stringify({ ...overrideValues?.h2, color: colour.canvas })}</pre>
        <hr />
        <h3 style={{ color: colour.canvas }}>Heading 3</h3>
        <pre>{JSON.stringify({ ...overrideValues?.h3, color: colour.canvas })}</pre>
        <hr />
        <h4 style={{ color: colour.canvas }}>Heading 4</h4>
        <pre>{JSON.stringify({ ...overrideValues?.h4, color: colour.canvas })}</pre>
        <hr />
        <h5 style={{ color: colour.canvas }}>Heading 5</h5>
        <pre>{JSON.stringify({ ...overrideValues?.h5, color: colour.canvas })}</pre>
        <hr />
        <h6 style={{ color: colour.canvas }}>Heading 6</h6>
        <pre>{JSON.stringify({ ...overrideValues?.h6, color: colour.canvas })}</pre>
        <hr />
        <div className={globalStyles.copyLarge}>Body Copy Large</div>
        <pre>{JSON.stringify(rawGlobalStyles.copyLarge)}</pre>
        <hr />
